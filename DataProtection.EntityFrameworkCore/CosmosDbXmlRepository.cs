...
/// </summary>
public class CosmosDbXmlRepository<TContext> : IXmlRepository
    where TContext : DbContext, IDataProtectionKeyContext
{
    ...
    /// <summary>
    /// Creates a new instance of the <see cref="CosmosDbXmlRepository{TContext}"/>.
    /// </summary>
    ...
    public CosmosDbXmlRepository(IServiceProvider services, ILoggerFactory loggerFactory)
    {
        ArgumentNullException.ThrowIfNull(loggerFactory);

        _logger = loggerFactory.CreateLogger<CosmosDbXmlRepository<TContext>>();
    ...
    public virtual IReadOnlyCollection<XElement> GetAllElements()
    {
        ...
        IEnumerable<XElement> GetAllElementsCore()
        {
            using (var scope = _services.CreateScope())
            {
                ...
                var keys = context.DataProtectionKeys.AsNoTracking().ToListAsync().Result;
    ...
    public void StoreElement(XElement element, string friendlyName)
    {
        ...
        {
            var context = scope.ServiceProvider.GetRequiredService<TContext>();
            var count = context.DataProtectionKeys.CountAsync().Result;
